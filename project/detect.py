...
def load_model(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    return (model, pt, onnx, stride, names, device, classify)


@torch.no_grad()
def run(model_parms, # loaded model
    ...
    model, pt, onnx, stride, names, device, classify = model_parms
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    savedir, img = save_path.rsplit("/", 1)
                    if len(det) > 0:
                        pred_path = os.path.join(savedir, "flower", img)
                        cv2.imwrite(pred_path, im0)
                    else:
                        pred_path = os.path.join(savedir, "noflower", img)
                        cv2.imwrite(pred_path, im0)
